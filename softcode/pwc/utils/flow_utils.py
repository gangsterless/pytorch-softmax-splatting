...
def show_compare(flow_out,flow_gt):
    flow_out_img = vis_flow(flow_out)
    flow_gt_img = vis_flow(flow_gt)
    _, (ax1, ax) = plt.subplots(1, 2, figsize=(6, 6))
    auto_show = True


    height, width = flow_gt_img.shape[:2]
    ax.set_ylim(height + 10, -10)
    ax.set_xlim(-10, width + 10)
    ax.axis('off')
    ax.set_title('flow_gt_img')

    ax1.set_ylim(height + 10, -10)
    ax1.set_xlim(-10, width + 10)
    ax1.axis('off')
    ax1.set_title("flow_out_img")

    ax.imshow(flow_gt_img.astype(np.uint8))
    ax1.imshow(flow_out_img.astype(np.uint8))
